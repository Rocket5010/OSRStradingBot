...
def main():
    ...
    print(f"ranking over {len(basket)} items (fetches timeseries each)...")
    rows = rank_over_items(c, basket, budget=50_000_000, min_candles=30)
    hdr = f"{'strategy':<16}{'gp/day':>12}{'profit':>16}{'trades':>8}{'win%':>6}{'maxDD':>7}"
    print(hdr)
    for r in rows:
